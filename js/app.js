    function renderMenu() {
      ...
      const topbarAvatar = document.querySelector('#accountBox .avatar');
      if (topbarAvatar) topbarAvatar.outerHTML = accountAvatarHtml(currentLoginName, '', 'topbarAvatar');
      ...
      document.getElementById('breadcrumb').textContent = current === 'profile' ? '个人中心' : current === 'projects' && garageSubpage !== 'list' ? garagePath : current === 'users' && userSubpage === 'detail' ? userPath : current === 'orders' && orderSubpage === 'detail' ? orderPath : current === 'passages' && passageSubpage === 'detail' ? passagePath : current === 'refunds' && refundSubpage === 'detail' ? refundPath : current === 'invoices' && invoiceSubpage === 'detail' ? invoicePath : current === 'clearing' && settlementSubpage === 'detail' ? settlementPath : (item ? item[2] : '项目管理');
    }
    ...
    const operationLogs = [
      ...
      ['ops02 / 陈露','订单管理','重试通行同步','SYNC20260828002','重新推送车辆月租通行权限至一路停车失败：车场编号无效','失败','2026-09-15 15:31'],
      ['finance01 / 张敏','退款处理','审批驳回','RF202609090007','驳回退款申请，原因"车辆仍在月租有效期内正常通行"','成功','2026-09-15 14:02'],
      ...
      ['ops02 / 陈露','订单管理','换绑车牌','CP20260828001','旧车牌浙A92Q8L 取消通行权限，新车牌浙A18N6P 已推送一路停车并更新权限','成功','2026-09-14 11:33'],
      ['finance01 / 张敏','支付与清分','手动清分','AS202608280016','对清分异常订单发起人工清分，金额 ¥1,200.00，清分状态：清分处理中','成功','2026-09-13 15:22'],
      ...
      ['ops02 / 陈露','订单管理','批量推送','已选 5 条订单','批量推送审核通过订单至一路停车，成功 4 条、失败 1 条','成功','2026-09-11 17:53'],
      ['admin','项目管理','新增收费项目','荣和家园 · 物业服务费','新增收费项目，标准 60.00 元/月，收款说明：物业商户清分','成功','2026-09-11 14:28'],
      ['finance01 / 张敏','退款处理','审批通过','RF202608250006','退款审批通过，金额 ¥1,200.00，订单状态已终止、通行失效','成功','2026-09-10 15:10'],
    ...
    const pages = {
      ...
      systemUsers() {
        ...
        return pageShell('账号管理', '', `${filters([['input','用户名'],['select','账号状态',['全部','启用','停用']]], '<button class="btn btn-primary" onclick="openCreateAccountModal()">新增账号</button><button class="btn" onclick="openBatchImportAccounts()" style="margin-left:10px">批量导入</button><button class="btn" onclick="batchDeleteAccounts()" style="margin-left:10px">批量删除</button>', 'filter-bar garage-filter-bar')}<section class="panel garage-table-panel account-table-panel"><div>${table([[' ','56px'],['头像','80px'],['用户名','165px'],['账号类型','145px'],['状态','115px'],['最后登录时间','170px'],['操作','280px']], accountRowsWithAvatar, (r)=>`<input type="checkbox" class="account-checkbox" data-username="${r[1]}" ${r[1] === 'admin' ? 'disabled' : ''} onclick="event.stopPropagation()">`, (r)=>`<button class="btn-text" onclick="openResetPasswordModal('${r[1]}')">重置密码</button>${r[1] === 'admin' ? '' : `<button class="btn-text${r[3] === '启用' ? ' danger' : ''}" onclick="toggleAccountStatus('${r[1]}')">${r[3] === '启用' ? '停用' : '启用'}</button><button class="btn-text danger" onclick="deleteAccount('${r[1]}')">删除</button>`}`)}</div></section>`);
      },
      logs() {
    ...
    Object.assign(pages, {
      ...
      profile() {
        const account = systemAccounts.find(item => item[0] === currentLoginName) || systemAccounts[0];
        const profile = profileOf(currentLoginName);
        const avatar = profileAvatars[currentLoginName];
        const infoRow = (label, field, editable) => { const value = editable ? (profile[field] || '') : currentLoginName; return `<div class="profile-row"><div class="profile-row-label">${label}</div><div class="profile-row-value"><span class="profile-text"${editable ? ` data-text-for="${field}"` : ''}>${value || '未填写'}</span>${editable ? `<input id="${field}" class="profile-input" value="${value}" placeholder="未填写" autocomplete="off">` : ''}</div><div class="profile-row-action"></div></div>`; };
        return pageShell('个人中心', '', `<section class="profile-card"><div class="profile-head"><span class="profile-avatar${avatar ? ' has-image' : ''}">${avatar ? `<img src="${avatar}" alt="头像">` : accountAvatarLetter(currentLoginName)}</span><div class="profile-head-main"><div class="profile-name" id="profileName">${profile.nickname || currentLoginName}</div><div class="profile-sub">${currentLoginName} / ${account[1]}</div></div><button class="btn" onclick="openAvatarModal()">更换头像</button></div></section><section class="profile-card" id="profileInfoSection"><div class="profile-section-head"><div class="profile-section-title">个人信息</div><div class="profile-actions"><button class="btn profile-edit-btn" onclick="startProfileEdit()">编辑资料</button><button class="btn profile-save-btn" onclick="saveProfile()">保存资料</button><button class="btn profile-cancel-btn" onclick="cancelProfileEdit()">取消</button></div></div><div id="profileError" class="approval-error profile-error"></div><div class="profile-rows is-form">${infoRow('登录账号', '', false)}${infoRow('用户名', 'nickname', true)}${infoRow('手机号', 'phone', true)}</div><div class="profile-section-head is-plain"><div class="profile-section-title">账号安全</div></div><div class="profile-rows"><div class="profile-row"><div class="profile-row-label">登录密码</div><div class="profile-row-value profile-password-value">${tag('已设置', 'info')}<span>用于保护账号登录安全</span></div><div class="profile-row-action"><button class="btn" onclick="openChangePasswordModal()">修改</button></div></div></div></section>`);
      }
    });
    render();
