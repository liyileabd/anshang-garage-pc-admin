    ...
    const pages = {
      ...
      logs() {
        return pageShell('操作日志', '', `${filters([['input','操作人/编号'],['select','操作模块',['全部','项目管理','订单管理','通行管理','财务管理','用户管理','系统管理']]], '', 'filter-bar garage-filter-bar')}<section class="panel garage-table-panel log-table-panel"><div>${table([['操作人','120px'],['操作模块','130px'],['操作类型','120px'],['操作对象','150px'],['操作内容摘要','240px'],['结果','80px'],['操作时间','150px'],['操作','90px']], operationLogs, (r,i)=>`<button class="btn-text" onclick="showLogDetail(${i})">查看</button>`)}</div></section>`);
      }
    };
