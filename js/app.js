    ...
    const invoiceExportFields = [['applicationNo','开票记录编号'],['orderNo','订单号'],['owner','申请用户'],['project','小区项目'],['type','发票类型'],['amount','开票金额'],['status','开票状态'],['applyDate','申请时间']];
    function batchExportInvoices() {
      const rows = invoiceRows();
      const fieldRows = invoiceExportFields.map(([key,label]) => `<label class="export-field-option"><input type="checkbox" name="invoiceExportField" value="${key}" checked><span>${label}</span></label>`).join('');
      showModal('自定义导出发票记录', `<div class="export-summary"><div class="export-summary-title">选择导出内容</div></div><div class="invoice-export-type" role="radiogroup" aria-label="票据类型"><label><input type="radio" name="invoiceExportType" value="all" checked>全部</label><label><input type="radio" name="invoiceExportType" value="ticket">非税票据</label><label><input type="radio" name="invoiceExportType" value="invoice">发票</label></div><div class="export-field-list">${fieldRows}</div>`);
      document.querySelector('#modalMask .modal')?.classList.add('invoice-export-modal');
      const confirm = document.getElementById('modalConfirmButton');
      if (confirm) { confirm.textContent = '确认导出'; confirm.onclick = () => { const type = document.querySelector('input[name="invoiceExportType"]:checked')?.value || 'all'; exportInvoices(rows.filter(row => type === 'all' || (type === 'ticket') === /票据/.test(row[4]))); }; }
    }
    function exportInvoices(rows) {
      const selected = [...document.querySelectorAll('input[name="invoiceExportField"]:checked')].map(input => input.value);
      if (!selected.length) { showModal('请选择导出字段', '至少选择一个字段后再导出。'); return; }
      const fields = invoiceExportFields.filter(([key]) => selected.includes(key));
      const lines = [fields.map(([,label]) => label), ...rows.map(row => fields.map(([key]) => ({applicationNo:row[0],orderNo:row[1],owner:row[2],project:row[3],type:row[4],amount:row[5],status:row[6],applyDate:row[7]}[key])))].map(row => row.map(csvCell).join(','));
      downloadCsv(`发票记录_${new Date().toISOString().slice(0,10)}.csv`, lines);
      hideModal();
    }
    const operationLogs = [
      ...
      ['finance01 / 张敏','财务管理','导出退款记录','筛选：已通过','导出退款记录 26 条，筛选条件"退款状态=已通过"','成功','2026-09-14 16:50'],
      ['property01 / 刘伟','订单管理','生成续费订单','AS20260812009','生成续费订单 AS202609150026，续费 12 个月，金额 ¥1,680.00','成功','2026-09-14 15:07'],
      ['ops02 / 陈露','通行管理','换绑审核通过','CP20260828001','审核通过：旧车牌浙A92Q8L 取消权限，新车牌浙A18N6P 待推送','成功','2026-09-14 11:33'],
      ...
      ['property01 / 刘伟','用户管理','导出用户列表','筛选：月租有效','导出用户数据 128 条，含姓名、手机号、车牌、月租状态','成功','2026-09-12 09:47'],
      ...
      ['admin','财务管理','导出发票记录','非税票据','导出非税票据 32 条，导出字段 8 项','成功','2026-09-09 16:14'],
    ...
    const pages = {
      ...
      users() {
      return pageShell('用户管理', '', `${filters([['input','姓名/手机号/车牌','', 'userKeyword', 'filterUsers'],['select','月租状态',['全部','月租有效','月租待生效','退款处理中','月租已过期','未办理'], 'userStatusFilter', 'filterUsers']], '<button class="btn" onclick="showModal(\'导出用户列表\',\'已按当前筛选条件生成用户列表导出任务。\')">导出</button>', 'filter-bar garage-filter-bar user-filter-bar', { query: 'filterUsers', reset: 'resetUserFilters' })}<section class="panel garage-table-panel"><div id="userTable">${userTableHtml(rentUsers)}</div></section>`);
      },
      vehicles() {
      ...
      orders() {
        ...
        return pageShell('订单查询', '', `${filters([['input','订单号/小区/车主/车牌','', 'orderKeyword', 'filterOrders'],['select','订单类型',['全部','新办','续费'], 'orderTypeFilter', 'filterOrders']], '<button class="btn" onclick="openOrderPushConfigModal()">订单推送配置</button><button class="btn" onclick="batchExportOrders()">批量导出</button>', 'filter-bar garage-filter-bar order-filter-bar', { query: 'filterOrders', reset: 'resetOrderFilters' })}<section class="panel garage-table-panel order-table-panel"><div id="orderTable">${orderTable(listRows)}</div></section>`);
      },
      passages() {
      ...
      plate() {
        ...
        return pageShell('车牌变更', '', panel('订单车辆操作记录', `${filters([['input','申请单/订单号/车主/车牌'],['select','审核状态',['全部','待审核','审核通过','未通过']],['select','同步状态',['全部','未同步','同步中','已同步']]])}<button class="btn" onclick="showModal('导出换绑记录','已按当前条件生成车辆变更记录导出任务。')">导出</button>${table([['申请单号','150px'],['原订单号','150px'],['车主','90px'],['原车牌','120px'],['新车牌','120px'],['审核状态','120px'],['同步状态','110px'],['操作','180px']], rows, (r)=>`<button class="btn-text" onclick="showModal('换绑审核','审核通过后将旧车牌取消权限，新车牌推送一路停车平台。')">审核</button><button class="btn-text" onclick="showModal('查看附件','本期仅预留车辆资料附件查看与人工复核入口，证件识别服务待评估。')">附件</button><button class="btn-text" onclick="showModal('重新同步','重新推送新旧车牌权限变更，并记录平台回执。')">同步</button>`)}`));
      },
      payments() {
