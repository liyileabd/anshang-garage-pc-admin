    ...
    Object.assign(pages, {
      ...
      profile() {
        ...
        const infoRow = (label, field, editable) => `<div class="profile-row"><div class="profile-row-label">${label}</div><div class="profile-row-value">${editable ? `<input id="${field}" class="profile-input" value="${profile[field] || ''}" placeholder="未填写" autocomplete="off">` : currentLoginName}</div><div class="profile-row-action"></div></div>`;
        return pageShell('个人中心', '', `<section class="profile-card"><div class="profile-head"><span class="profile-avatar${avatar ? ' has-image' : ''}">${avatar ? `<img src="${avatar}" alt="头像">` : accountAvatarLetter(currentLoginName)}</span><div class="profile-head-main"><div class="profile-name" id="profileName">${profile.name || currentLoginName}</div><div class="profile-sub">${currentLoginName} / ${account[1]}</div><div class="profile-tags">${tag(profile.company, 'info')}${tag(profile.position, 'info')}${tag(account[2])}</div></div><button class="btn" onclick="openAvatarModal()">更换头像</button></div></section><section class="profile-card"><div class="profile-section-head"><div class="profile-section-title">个人信息</div><button class="btn" onclick="saveProfile()">保存资料</button></div><div id="profileError" class="approval-error profile-error"></div><div class="profile-rows">${infoRow('登录账号', '', false)}${infoRow('姓名', 'name', true)}${infoRow('用户名', 'nickname', true)}${infoRow('手机号', 'phone', true)}${infoRow('邮箱', 'email', true)}</div><div class="profile-section-head is-plain"><div class="profile-section-title">账号安全</div></div><div class="profile-rows"><div class="profile-row"><div class="profile-row-label">登录密码</div><div class="profile-row-value profile-password-value">${tag('已设置', 'info')}<span>用于保护账号登录安全</span></div><div class="profile-row-action"><button class="btn" onclick="openChangePasswordModal()">修改</button></div></div></div></section>`);
      }
    });
    render();
